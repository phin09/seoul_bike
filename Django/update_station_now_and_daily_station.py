        ...
        for item in api_dict:
            ...
            try:  # 대여소가 api에도 table area에도 있는 경우 -> update
                ...
                try:  # update table station_now
                    station = Stations.objects.get(dataId=dataId)
                    stationNow = StationNow.objects.create(
            ...
            except Stations.DoesNotExist:   # 대여소가 api에는 있지만 table area에 없는 경우 -> pass
                pass
...
#         check = Stations.objects.filter(stationCode=stationCode)
#         if len(check) > 0:
#             dataId = get_object_or_404(, stationCode=stationCode)
#             parkingBikeTotCnt = int(item.parkingBikeTotCnt)
